...
def handle_scan_command(args: argparse.Namespace) -> None:
    ...
    scanner = AWSScanner(
        ...
        args=args  # Pass the args to the scanner
    )
    ...
    if args.output == "json":
        ...
        # Print table header
        print("\n{:<40} {:<15} {:<20} {:<40} {:<10}".format(
            "RESOURCE NAME", "RESOURCE TYPE", "AWS ACCOUNT", "ENV VARIABLE", "SEVERITY"
        ))
        print("-" * 130)

        # Print table rows
        for result in results:
            print("{:<40} {:<15} {:<20} {:<40} {:<10}".format(
                result["resource_name"][:40],  # Increased from 20 to 40
                result["resource_type"],
                result["account_id"],
                result["environment_variable"][:40],  # Increased from 30 to 40
